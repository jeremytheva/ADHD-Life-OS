...
test('pending edit owns the editor until its write settles', () => {
  assert.match(source, /const handleStartEdit = \(item\) => \{\s*if \(editPending \|\| deletingIdsRef\.current\.has\(item\.id\)\) return/)
  ...
  assert.match(source, /onClick=\{\(\) => handleStartEdit\(item\)\}\s*disabled=\{editPending \|\| deletingIds\.has\(item\.id\)\}/)
})
