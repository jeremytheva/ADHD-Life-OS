...
test('Brain Inbox exposes Capture and Organize modes as a labelled stateful control group', async () => {
  ...
  assert.match(source, /onClick=\{\(\) => handleModeChange\('capture'\)\}/)
  assert.match(source, /onClick=\{\(\) => handleModeChange\('organize'\)\}/)
  assert.match(source, /disabled=\{modeChangePending\}/)
})
