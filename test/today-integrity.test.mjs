...
test('Today task completion is serialized and distinguishes write failure from refresh failure', async () => {
  ...
  assert.match(todaySource, /const pendingTaskRef = useRef\(null\)/)
  assert.match(todaySource, /if \(pendingTaskRef\.current\) return/)
  assert.match(todaySource, /const acceptedTaskId = taskId/)
  assert.match(todaySource, /await taskService\.completeTask\(acceptedTaskId\)/)
  ...
  assert.match(todaySource, /pending=\{block\.ref_type === 'task' && taskMutationPending\}/)
