...
test('quick capture preserves only unsaved tasks after a partial write failure', async () => {
  ...
  assert.match(modalSource, /const acceptedItems = \[\.\.\.validItems\]/)
  assert.match(modalSource, /const result = await onSave\(acceptedItems\)/)
...
test('chore detail preserves checked steps and gates celebration on confirmed completion', async () => {
  ...
  assert.match(source, /const closeLocked = completing \|\| showCelebration/)
  assert.match(source, /if \(completionPendingRef\.current \|\| showCelebration\) return/)
  assert.match(source, /onEscape: closeLocked \? undefined : handleClose/)
...
test('task mutations preserve retry context and distinguish reconciliation failure', async () => {
  ...
  assert.match(listSource, /pending=\{mutationPending\}/)
