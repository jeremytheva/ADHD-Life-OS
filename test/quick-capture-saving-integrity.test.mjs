...
test('Quick Capture guards and announces an unresolved save', async () => {
  ...
  assert.match(source, /const handleCancel = \(\) => \{[\s\S]*?if \(submitOwnerRef\.current !== null\) return[\s\S]*?onCancel\(\)/)
  assert.match(source, /onEscape: handleCancel/)
  assert.match(source, /const handleAddItem = \(\) => \{[\s\S]*?if \(submitOwnerRef\.current !== null\) return/)
  assert.match(source, /const handleRemoveItem = \(index\) => \{[\s\S]*?if \(submitOwnerRef\.current !== null\) return/)
