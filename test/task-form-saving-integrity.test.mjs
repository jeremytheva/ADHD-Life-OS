...
test('Task Form freezes mutation and dismissal while saving', async () => {
  ...
  assert.match(source, /const handleSubmit = async \(e\) => \{[\s\S]*?if \(saving \|\| submitOwnerRef\.current\) return[\s\S]*?await onSave\(/)
  assert.match(source, /const handleChange = \(field, value\) => \{\s*if \(saving \|\| submitOwnerRef\.current\) return/)
  assert.match(source, /const handleCancel = \(\) => \{\s*if \(saving \|\| submitOwnerRef\.current\) return/)
