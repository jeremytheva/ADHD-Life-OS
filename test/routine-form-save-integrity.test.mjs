...
test('Routine Form delegates routine and step persistence as one guarded save', async () => {
  ...
  assert.match(source, /const handleSubmit = async \(e\) => \{[\s\S]*?if \(saving\) return[\s\S]*?const submittedRoutine = \{[\s\S]*?steps: steps\.map\(\(step\) => \(\{ \.\.\.step \}\)\)[\s\S]*?setSaving\(true\)[\s\S]*?await onSave\(submittedRoutine\)[\s\S]*?finally \{[\s\S]*?setSaving\(false\)/)
  assert.match(source, /const handleChange = \(field, value\) => \{[\s\S]*?if \(saving\) return/)
  assert.match(source, /const handleStepChange = \(index, field, value\) => \{[\s\S]*?if \(saving\) return/)
