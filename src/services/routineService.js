...
const step = (routineId, value, index) => ({ routine_id: routineId, name: value.name, description: value.description || '', duration_minutes: value.duration_minutes, order_index: index, is_essential: value.is_essential || false, preferred_time: value.preferred_time || null })
...
export const routineService = {
  ...
  async createRoutine(data) { const id = userId(); const routine = await repositories.routines.create({ user_id: id, name: data.name, description: data.description || '', repeat_pattern: data.repeat_pattern || 'daily', is_active: data.is_active !== false, mode: data.mode || null, category: data.category || null, tags: data.tags || [] }); const steps = await Promise.all((data.steps || []).map((value, index) => repositories.routineSteps.create({ user_id: id, ...step(routine.id, value, index) }))); return { ...routine, routine_steps: steps } },
