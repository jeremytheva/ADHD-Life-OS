...
export const routineService = {
  async getRoutines() {
    ...
    if (!isSupabaseConfigured()) {
      ...
      return routines.map(routine => ({
        ...routine,
        steps: steps.filter(s => s.routine_id === routine.id)
  ...
  async updateRoutine(routineId, updates) {
    ...
    if (!isSupabaseConfigured()) {
      // Use mock data
      const routines = getMockRoutines();
      const index = routines.findIndex(r => r.id === routineId && r.user_id === userId);
  ...
  async deleteRoutine(routineId) {
    ...
    if (!isSupabaseConfigured()) {
      // Use mock data
      const routines = getMockRoutines();
      const filtered = routines.filter(r => !(r.id === routineId && r.user_id === userId));
