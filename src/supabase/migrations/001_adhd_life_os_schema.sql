/*
 # ADHD Life-OS Database Schema

 This migration creates the complete database schema for the ADHD Life-OS MVP.

 1. New Tables
 - `users` - Maps to Supabase auth users
 - `user_roles` - User role assignments (adult, teen, workplace, parent)
 - `user_preferences` - DaySkeleton configuration and user settings
 - `tasks` - Task management with essential flag and duration
 - `routines` - Routine definitions with repeat patterns
 - `routine_steps` - Individual steps within routines
 - `events` - External calendar events (Google Calendar sync)
 - `blocks` - Generated schedule blocks from the scheduling engine
 - `strategy_suggestion_log` - AI suggestion tracking (future feature)
 - `audit_log` - System audit trail

 2. Security
 - Row Level Security (RLS) enabled on all tables
 - Self-access policies for authenticated users
