      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-slate-50 rounded-lg p-4"><div className="flex items-center gap-2 text-slate-600 mb-1"><SafeIcon icon={FiClock} className="w-4 h-4" aria-hidden="true" /><span className="text-sm font-medium">Duration</span></div><div className="text-2xl font-bold text-slate-900">{task.estimated_duration} min</div>{(task.prep_time > 0 || task.cleanup_time > 0) && <div className="text-xs text-slate-600 mt-1">{task.prep_time > 0 && `+${task.prep_time}m prep`}{task.prep_time > 0 && task.cleanup_time > 0 && ', '}{task.cleanup_time > 0 && `+${task.cleanup_time}m cleanup`}</div>}</div>
            <div className="bg-slate-50 rounded-lg p-4"><div className="flex items-center gap-2 text-slate-600 mb-1"><SafeIcon icon={FiCalendar} className="w-4 h-4" aria-hidden="true" /><span className="text-sm font-medium">Frequency</span></div><div className="text-lg font-bold text-slate-900">{getFrequencyLabel(task.frequency)}</div><div className="text-xs text-slate-600 mt-1">Due {format(parseISO(task.next_due_date), 'MMM d')}</div></div>
          </div>

          {task.required_items?.length > 0 && <div className="mb-6"><div className="flex items-center gap-2 mb-3"><SafeIcon icon={FiPackage} className="w-5 h-5 text-slate-600" aria-hidden="true" /><h3 id={requiredItemsHeadingId} className="font-medium text-slate-900">What You'll Need</h3></div><ul className="flex flex-wrap gap-2" aria-labelledby={requiredItemsHeadingId}>{task.required_items.map((item, index) => <li key={index} className="px-3 py-1 bg-blue-50 text-blue-700 rounded-full text-sm">{item}</li>)}</ul></div>}

          {task.checklist?.length > 0 && <div><div className="flex items-center gap-2 mb-3"><SafeIcon icon={FiList} className="w-5 h-5 text-slate-600" aria-hidden="true" /><h3 id={stepsHeadingId} className="font-medium text-slate-900">Steps</h3></div><div className="space-y-2" role="group" aria-labelledby={stepsHeadingId}>{task.checklist.map((item, index) => <motion.button type="button" key={index} onClick={() => handleToggleChecklistItem(index)} disabled={closeLocked} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }} aria-pressed={Boolean(checklistState[index])} className={`w-full text-left p-4 rounded-lg border-2 transition-all disabled:cursor-not-allowed ${checklistState[index] ? 'bg-green-50 border-green-300' : 'bg-white border-slate-200 hover:border-purple-300'}`}><div className="flex items-center gap-3"><div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${checklistState[index] ? 'bg-green-500' : 'bg-slate-200'}`}>{checklistState[index] && <SafeIcon icon={FiCheck} className="w-4 h-4 text-white" aria-hidden="true" />}</div><div className="flex items-center gap-2 flex-1"><span className="text-slate-400 font-medium">{index + 1}.</span><span className={checklistState[index] ? 'text-green-900 line-through' : 'text-slate-900'}>{item}</span></div></div></motion.button>)}</div></div>}

          {task.completion_count > 0 && <div className="mt-6 p-4 bg-purple-50 rounded-lg border border-purple-200"><div className="flex items-center gap-2 mb-2"><SafeIcon icon={FiTrendingUp} className="w-4 h-4 text-purple-600" aria-hidden="true" /><span className="text-sm font-medium text-purple-900">Your Progress</span></div><p className="text-sm text-purple-800">You've completed this {task.completion_count} time{task.completion_count !== 1 ? 's' : ''}! {task.last_completed && <> Last done {format(parseISO(task.last_completed), 'MMM d')}.</>}</p></div>}
        </div>
        ...
        <div className="p-6 border-t border-slate-200 bg-slate-50">
          ...
          >
            {completing ? <><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" aria-hidden="true"></div><span>Confirming completion...</span></> : <><SafeIcon icon={FiCheck} className="w-5 h-5" aria-hidden="true" />Mark as Complete</>}
          </button>
