  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="housework-setup-title"
        aria-busy={saving}
        tabIndex={-1}
      ...
      >
        <div className="bg-gradient-to-r from-purple-500 to-indigo-600 p-6 text-white">
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm">{selectedTasks.size} task{selectedTasks.size !== 1 ? 's' : ''} selected</div>
            <button
              type="button"
              onClick={handleSelectStarterSet}
              disabled={saving}
              className="px-4 py-2 bg-white bg-opacity-20 hover:bg-opacity-30 rounded-lg text-sm transition-colors disabled:opacity-50"
            >
            ...
            {filteredTemplates.map((template, index) => {
              ...
              return (
                <motion.button
                  ...
                  className={`text-left p-4 rounded-lg border-2 transition-all disabled:cursor-not-allowed ${isSelected ? 'bg-purple-50 border-purple-300' : 'bg-white border-slate-200 hover:border-purple-200'}`}
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    {template.is_essential && <span className="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-medium">Essential</span>}
                  </div>
