  ...
  const renderApplyDropdown = () => (
    <div className="relative">
      ...
      >
        ...
        {!isApplied && !isApplying && <SafeIcon icon={FiChevronDown} className="w-3 h-3" />}
      </button>
        ...
        <>
          ...
          <motion.div role="menu" aria-label={`Apply ${templateName}`} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20" onKeyDown={handleApplyMenuKeyDown}>
            <button ref={(element) => { applyMenuItemRefs.current[0] = element }} type="button" role="menuitem" onClick={() => { onDirectApply(); closeApplyMenu() }} className="w-full px-4 py-2 text-left text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center gap-2">
              <SafeIcon icon={FiPlus} className="w-4 h-4" /><span>Apply Directly</span>
            </button>
            <button ref={(element) => { applyMenuItemRefs.current[1] = element }} type="button" role="menuitem" onClick={onEditBeforeApply} className="w-full px-4 py-2 text-left text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center gap-2">
              <SafeIcon icon={FiEdit3} className="w-4 h-4" /><span>Edit Before Applying</span>
            </button>
          </motion.div>
        </>
      )}
  ...
  const content = (
    <>
      <div className="flex items-start justify-between mb-3">
        <div className={`p-2 rounded-lg ${isRoutine ? 'bg-purple-100' : 'bg-green-100'}`}><SafeIcon icon={isRoutine ? FiRefreshCw : FiCheckSquare} className={`w-5 h-5 ${isRoutine ? 'text-purple-600' : 'text-green-600'}`} /></div>
        {isApplied && <span className="bg-green-100 text-green-700 px-2 py-1 rounded-full text-xs flex items-center gap-1"><SafeIcon icon={FiCheck} className="w-3 h-3" />Applied</span>}
      </div>
      ...
      <div className="flex items-center gap-3 text-xs text-slate-500 mb-4">
        ...
        <div className="flex items-center gap-1"><SafeIcon icon={FiClock} className="w-3 h-3" /><span>{totalDuration} min</span></div>
      </div>
      <div className="flex gap-2">
        <button type="button" onClick={onPreview} disabled={isApplying} className="flex-1 px-3 py-2 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors text-sm flex items-center justify-center gap-2 disabled:opacity-50"><SafeIcon icon={FiEye} className="w-4 h-4" />Preview</button>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1"><h4 className="font-medium text-slate-900">{templateName}</h4><span className={`px-2 py-0.5 rounded-full text-xs ${getCategoryColor(template.category)}`}>{template.category}</span>{isApplied && <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs flex items-center gap-1"><SafeIcon icon={FiCheck} className="w-3 h-3" />Applied</span>}</div>
              <p className="text-sm text-slate-600 mb-2">{template.description}</p>
              <div className="flex items-center gap-4 text-xs text-slate-500">{isRoutine && <span>{template.steps?.length || 0} steps</span>}<div className="flex items-center gap-1"><SafeIcon icon={FiClock} className="w-3 h-3" /><span>{totalDuration} min</span></div><div className="flex flex-wrap gap-1">{template.tags?.slice(0, 3).map(tag => <span key={tag} className="text-xs text-slate-500">#{tag}</span>)}</div></div>
            </div>
          </div>
          <div className="flex items-center gap-2"><button type="button" onClick={onPreview} disabled={isApplying} aria-label={`Preview ${templateName}`} className="p-2 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors disabled:opacity-50" title="Preview"><SafeIcon icon={FiEye} className="w-4 h-4" /></button>{renderApplyDropdown()}</div>
        </div>
      </motion.div>
    )
  }
